...
import { Show, splitProps, type Component, type ComponentProps, type ValidComponent } from "solid-js";
...
const OTPField = <T extends ValidComponent = "div">(props: DynamicProps<T, OTPFieldProps<T>>) => {
  ...
  return (
    <OtpField
      class={cn("flex items-center gap-2 disabled:cursor-not-allowed has-[:disabled]:opacity-50", local.class)}
      {...others}
    />
  );
};
